<?php 
    ...
    if(isset($_GET['save'])){
        ...
        $sql_getId_projet = "SELECT p.id FROM affectations a 
                     JOIN projet p ON a.id_empl = p.id  
                     WHERE pr_name = '{$_GET['name_projet']}'";
        ...
        while($row=mysqli_fetch_assoc($result_getId_employee)){
            if (isset($_GET[$row['id_empl']])) {
                $sql = "INSERT INTO historique (id_projet, id_empl, présence) 
                         VALUES ('$idprojet', '{$row['id_empl']}', 'present')";
                mysqli_query($conn,$sql);
            
        }else{
            $sql = "INSERT INTO historique (id_projet, id_empl, présence) 
            VALUES ('$idprojet', '{$row['id_empl']}', 'absent')";
            mysqli_query($conn,$sql);
        }
